...
def test_map_value_prim():
    ll2mu = LL2MuValueMapper(LL2MuTypeMapper())
    assert ll2mu.map(10) == mutype.mu_int64(10)
    assert ll2mu.map(rffi.INT._type(50)) == mutype.mu_int32(50)
    ...
    assert ll2mu.map(rffi.INT._type(-50)) == mutype.mu_int32(4294967246)

    assert ll2mu.map(rarithmetic.r_singlefloat(1.5)) == mutype.mu_float(1.5)

    from rpython.rlib.objectmodel import malloc_zero_filled
    assert ll2mu.map(malloc_zero_filled) == mutype.mu_int64(0)

    assert ll2mu.map('c') == mutype.mu_int8(ord('c'))

def test_map_value_adrofs():
    from rpython.translator.mu.layout import mu_sizeOf, mu_offsetOf, mu_hybsizeOf
    ll2mu = LL2MuValueMapper(LL2MuTypeMapper())
    assert ll2mu.map(llmemory.sizeof(lltype.Char)) == mutype.mu_int64(1)
    S = lltype.GcStruct("Point", ('x', lltype.Signed), ('y', lltype.Signed))
    MuS = ll2mu.ll2mu_t.map(S)
    assert ll2mu.map(llmemory.sizeof(S)) == mutype.mu_int64(mu_sizeOf(MuS))
    assert ll2mu.map(llmemory.offsetof(S, 'y')) == mu_offsetOf(MuS, 'y')

    A = lltype.GcArray(lltype.Char)
    MuA = ll2mu.ll2mu_t.map(A)
    assert ll2mu.map(llmemory.itemoffsetof(A, 10)) == mu_hybsizeOf(MuA, 10)
...
def test_map_value_stt():
    ...
    mupt3d = ll2mu.map(pt3d)
    assert mutype.mutypeOf(mupt3d) == ll2mu.ll2mu_t.map(Point3D)
    assert isinstance(mupt3d.super, mutype._mustruct)
    assert mutype.mutypeOf(mupt3d.super) == ll2mu.ll2mu_t.map(Point)
    mupt4d = mupt3d._parentstructure()
    assert isinstance(mupt4d, mutype._mustruct)
    assert mutype.mutypeOf(mupt4d) == ll2mu.ll2mu_t.map(Point4D)
...
def test_map_value_ptr():
    ...
    assert ll2mu.map(nullptr)._is_null()

    p = lltype.malloc(Point, zero=True)
    r = ll2mu.map(p)
    ll2mu.resolve_ptrs()
    assert isinstance(r, mutype._muref)
    assert mutype.mutypeOf(r._obj) == ll2mu.ll2mu_t.map(Point)
