...
#
#     graph = graph.startblock.operations[0].args[0].value._obj.graph
#     print_graph(graph)
#
#     cnst = graph.startblock.operations[0].args[0]
#     cnst_norm = normalise_constant(cnst)
#     assert cnst_norm.concretetype.TO.OF.TO._name != 'object'
#

def test_graph_normalise_constant():
    from pyhaskell.interpreter.jscparser import parse_js
    t, _, graph = gengraph(parse_js, [str])
    ...
    g = prepare(t.graphs, graph)[0]
    cnst_0 = g.startblock.operations[0].args[0]
    cnst_1 = g.startblock.operations[1].args[1]
    assert hash(cnst_0) == hash(cnst_1)