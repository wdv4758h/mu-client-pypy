...
class MuExceptionTransformer:
    ...
    def exctran(self, g):
        ...
        # - mu_throw it
        excblk = g.exceptblock
        excblk.operations, excdata = self.get_pack_ops(excblk.inputargs[0], excblk.inputargs[1])
        dummy_var = Variable()
        dummy_var.concretetype = lltype.Void
        excblk.operations.append(SpaceOperation('mu_throw', [excdata], dummy_var))
    ...
    def exctran_block(self, excblk, g):
        def _wrap_llexitcase(llec):
            if llec and isinstance(llec, _ptr) and llec._TYPE == self.exctype_llt:
                llec = Constant(llec, llec._TYPE)
            return llec
        ...
        def _create_catch_block(args):
            ...
            excdataptr = Variable("excdataptr")
            excdataptr.concretetype = self.ptr_excdata_llt
            blk_catch.operations = (SpaceOperation('cast_pointer', [Constant(self.ptr_excdata_llt), excobjptr], excdataptr), )
        ...
        def _create_compare_blocks(lnks):
            pass

        if len(excblk.exits) > 2:
            lnks = excblk.exits[1:]
            args = _collect_args(lnks)
            catblk, excdataptr = _create_catch_block(args)
            excblk.exits = (excblk.exits[0], _link(excblk, catblk, args))
            cmp_chain = _create_compare_blocks(lnks)
            catblk.exits = (_link(catblk, cmp_chain, [excdataptr, _wrap_llexitcase(lnks[0].llexitcase)] + args), )
    ...
    def _get_unpack_type_op(self, var_excdata):
        exc_t = Variable('exc_t')
        exc_t.concretetype = self.exctype_llt
        return SpaceOperation('direct_call', [self.mupyexc_gettype, var_excdata], exc_t), exc_t

    def _get_unpack_val_op(self, var_excdata):
        exc_v = Variable('exc_v')
        exc_v.concretetype = self.excval_llt
        SpaceOperation('direct_call', [self.mupyexc_getvalue, var_excdata], exc_v), exc_v
