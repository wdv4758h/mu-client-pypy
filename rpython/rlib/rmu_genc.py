...
class MuIRBuilder:
    ...
    def gen_sym(self, name=None):
        # type: (str) -> MuID
        var_id = CVar('MuID', 'id')
        apilog.log(CCall('gen_sym', [self._bldr, CStr(name) if name else NULL], var_id, self._bldr))
        return var_id

    def new_type_int(self, id, len):
        # type: (MuID, int) -> None
        apilog.log(CCall('new_type_int', [self._bldr, id, len], None, self._bldr))

    def new_type_uptr(self, id, ty):
        # type: (MuID, MuTypeNode) -> None
        apilog.log(CCall('new_type_uptr', [self._bldr, id, ty], None, self._bldr))
    ...
    def new_binop(self, id, result_id, optr, ty, opnd1, opnd2, exc_clause=MU_NO_ID):
        ...
        apilog.log(CCall('new_binop', [self._bldr, id, result_id, optr,
                                       ty, opnd1, opnd2,
                                       exc_clause],
                         None, self._bldr))
    ...
    def new_cmp(self, id, result_id, optr, ty, opnd1, opnd2):
        ...
        apilog.log(CCall('new_cmp', [self._bldr, id, result_id, optr,
                                     ty, opnd1, opnd2],
                         None, self._bldr))

    def new_branch(self, id, dest):
        # type: (MuID, MuDestClause) -> None
        apilog.log(CCall('new_branch', [self._bldr, id, dest], None, self._bldr))

    def new_branch2(self, id, cond, if_true, if_false):
        # type: (MuID, MuVarNode, MuDestClause, MuDestClause) -> None
        apilog.log(CCall('new_branch2', [self._bldr, id, cond,
                                         if_true, if_false],
                         None, self._bldr))
    ...
    def new_call(self, id, result_ids, sig, callee, args, exc_clause=MU_NO_ID, keepalive_clause=MU_NO_ID):
        ...
        apilog.log(CCall('new_call', [self._bldr, id,
    ...
    def new_ret(self, id, rvs):
        ...
        apilog.log(CCall('new_ret', [self._bldr, id, rvs_arr, rvs_sz], None, self._bldr))
    ...
    def new_store(self, id, is_ptr, ord, refty, loc, newval, exc_clause=MU_NO_ID):
        ...
        apilog.log(CCall('new_store', [self._bldr, id, 'true' if is_ptr else 'false',
                                       ord, refty,
                                       loc, newval,
                                       exc_clause],
                         None, self._bldr))
