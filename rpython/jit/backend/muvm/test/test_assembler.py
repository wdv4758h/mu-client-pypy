from rpython.jit.backend.muvm import conditions as c, registers as r
from rpython.jit.backend.muvm.assembler import AssemblerMu
from rpython.jit.backend.muvm.test.support import run_asm
...
class TestRunningAssembler(object):
    ...
    def test_make_operation_list(self):
        i = rop.INT_ADD
        from rpython.jit.backend.muvm import assembler
        assert assembler.asm_operations[i] \
            is AssemblerMu.emit_op_int_add.im_func
    ...
    def test_division(self):
        ...
        div_addr = rffi.cast(lltype.Signed, Mu_int_div)
